/********************************************************************
 ...
 internal byte-sized buffer using a counter to know when to write or  a
...
int bits_io_read_bit (BitsIOFile *bfile) {
  ...
  if(bfile->bitsLeft == 0){
    bfile->numBytes++;
    int read_bytes = fgetc(bfile->fp);
    if(read_bytes == -1){
      return -1;
    }

    else{
      bfile->byte = read_bytes;
      bfile->bitsLeft = 8;
    }
  }

  int temp = bfile->byte >> 7;
  bfile->byte = bfile->byte << 1;
  bfile->bitsLeft = bfile->bitsLeft - 1;
